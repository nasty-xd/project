...
if (createMapForm) {
    ...
    createMapForm.addEventListener('submit', async function(e) {
        ...
        if (!response.ok) {
            alert(newMap.message || 'Map was not created');
            return;
        }

        window.location.href =
            'map.html?id=' + newMap._id;
    });
}
...
async function loadMarkersForMap(mapId) {
    markerData = [];
    markers = [];
    ...
    const currentMap = await response.json();
...
async function saveCurrentMapMarkers() {
    const mapId = getQueryParam('id');
    const token = localStorage.getItem('token');

    await fetch(`http://localhost:3000/api/maps/${mapId}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            Authorization: 'Bearer ' + token
...
async function renderMaps() {
    ...
    if (!response.ok) {
        alert(maps.message || 'Maps loading error');
        return;
    }
...
async function deleteMap(mapId) {
    ...
    const response = await fetch(
        `http://localhost:3000/api/maps/${mapId}`,
        {
            method: 'DELETE',
            headers: {
                Authorization: 'Bearer ' + token
...
async function renameMap(mapId) {
    const currentMap = loadedMaps.find(map =>
        String(map._id) === String(mapId)
    );
    ...
    if (!response.ok) {
        alert(data.message || 'Rename error');
        return;
    }
...
async function loadMapTitle(mapId) {
    const token = localStorage.getItem('token');
